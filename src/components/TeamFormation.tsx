...
export default function TeamFormation({ players, onTeamsFormed, roomOwner, playerId }: TeamFormationProps) {
    ...
    useEffect(() => {
        ...
        const uniquePlayers = players.filter((player, index, self) => self.findIndex((p) => p.id === player.id) === index);
        ...
        if (!canStartGame && teams.some((team) => team.players.length > 0)) {
            ...
            const invalidTeams = teams
                .filter((team) => team.players.length > 0 && team.players.length < 2)
                .map((team) => team.name);

            if (invalidTeams.length > 0) {
                setErrorMessage(
                    `Teams need at least 2 players each. ${invalidTeams.join(", ")} ${
                        invalidTeams.length === 1 ? "needs" : "need"
                    } more players.`
                );
            } else {
        ...
        <div className="max-w-3xl mx-auto p-4 bg-white/90 backdrop-blur-sm rounded-xl shadow-lg h-[80vh] max-h-[600px] flex flex-col">
            <div className="mb-2">
                ...
                <p className="text-gray-700 text-center text-sm">
                    At least 2 teams with at least 2 players each are required
                </p>
            </div>
                        ...
                        {teams.map((team) => (
                            ...
                            >
                                ...
                                <ul className="space-y-1">
                                    {(roomOwner ? team.players : team.players.filter((p) => p.id === playerId)).map(
                                        (player) => (
                                            <li
                                                key={`team-${team.id}-player-${player.id}`}
                                                className="flex items-center justify-between p-1 bg-gray-50 rounded-md text-sm"
                                            >
                                    ...
                                    {team.players.length === 0 && (
                                        <li className="text-xs text-gray-500 p-1 bg-gray-50 rounded-md">
                ...
                <div className="space-y-2">
                    ...
                    <div className="space-y-2 overflow-y-auto pr-1" style={{ maxHeight: "calc(100% - 2rem)" }}>
                        {unassignedPlayers.map((player, index) => (
                            <div
                                key={`unassigned-${player.id}-${index}`}
                                className="p-2 border border-gray-200 rounded-lg bg-white hover:shadow-md transition-all"
                            >
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="flex items-center space-x-1">
                                        <select
                                            ...
                                            onChange={(e) =>
                                                setSelectedTeams({ ...selectedTeams, [player.id]: e.target.value })
                                            }
